...
module.exports.buildPCBlockStatesArray = function (registry, blockStates) {
  ...
  if (registry.supportFeature('usesBlockStates')) {
    ...
    const states = Object.keys(mcData.legacy.pc.blocks).map(k => k.split(':'))
    for (const [type, meta] of states) {
      const stateId = type << 4 | meta
      const block = Block.fromStateId(stateId, 0)
      const variants = getModelVariants(block, blockStates)
      blockStateVariantsByStateId[block.stateId] = variants
    }
  }
  return blockStateVariantsByStateId
}
